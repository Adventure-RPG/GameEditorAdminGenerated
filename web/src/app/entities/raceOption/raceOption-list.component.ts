...
import { RaceOptionDetailComponent } from './raceOption-detail.component';
import { RaceOptionService } from './raceOption.service';

@Component({
    moduleId: module.id,
	templateUrl: 'raceOption-list.component.html',
	selector: 'raceOption-list'
})
export class RaceOptionListComponent {

    @Input() header = "RaceOptions...";
    ...
    raceOptionToDelete : RaceOption;

    // basic search criterias (visible if not in 'sub' mode)
    example : RaceOption = new RaceOption();

    // list is paginated
    currentPage : PageResponse<RaceOption> = new PageResponse<RaceOption>(0,0,[]);


    constructor(private router : Router,
        private raceOptionService : RaceOptionService,
    ...
    loadPage(event : LazyLoadEvent) {
        this.raceOptionService.getPage(this.example, event).
            subscribe(
    ...
    onRowSelect(event : any) {
        ...
        this.router.navigate(['/raceOption', id]);
    }
    ...
    addNew() {
        if (this.sub) {
            ...
            this.router.navigate(['/raceOption', 'new']);
        }
    }

    showDeleteDialog(rowData : any) {
        let raceOptionToDelete : RaceOption = <RaceOption> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(raceOptionToDelete);
            }
        });
    }

    private delete(raceOptionToDelete : RaceOption) {
        let id =  raceOptionToDelete.id;

        this.raceOptionService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(raceOptionToDelete);
