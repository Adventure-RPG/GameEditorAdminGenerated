...
import { CharacterLineComponent } from '../character/character-line.component';
...
export class CharacteristicListComponent {
    ...
    example : Characteristic = new Characteristic();

    // list is paginated
    currentPage : PageResponse<Characteristic> = new PageResponse<Characteristic>(0,0,[]);
    ...
    loadPage(event : LazyLoadEvent) {
        this.characteristicService.getPage(this.example, event).
            subscribe(
    ...
    onRowSelect(event : any) {
        ...
        this.router.navigate(['/characteristic', id]);
    }
    ...
    addNew() {
        if (this.sub) {
            ...
            this.router.navigate(['/characteristic', 'new']);
        }
    }

    showDeleteDialog(rowData : any) {
        let characteristicToDelete : Characteristic = <Characteristic> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(characteristicToDelete);
            }
        });
    }

    private delete(characteristicToDelete : Characteristic) {
        let id =  characteristicToDelete.id;

        this.characteristicService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(characteristicToDelete);
