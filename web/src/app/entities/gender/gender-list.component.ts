...
export class GenderListComponent {
    ...
    genderToDelete : Gender;

    // basic search criterias (visible if not in 'sub' mode)
    example : Gender = new Gender();

    // list is paginated
    currentPage : PageResponse<Gender> = new PageResponse<Gender>(0,0,[]);
    ...
    loadPage(event : LazyLoadEvent) {
        this.genderService.getPage(this.example, event).
            subscribe(
    ...
    addNew() {
        if (this.sub) {
            ...
            this.router.navigate(['/gender', 'new']);
        }
    }

    showDeleteDialog(rowData : any) {
        let genderToDelete : Gender = <Gender> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(genderToDelete);
            }
        });
    }

    private delete(genderToDelete : Gender) {
        let id =  genderToDelete.id;

        this.genderService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(genderToDelete);
