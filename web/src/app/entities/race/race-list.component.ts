...
import { RaceOptionLineComponent } from '../raceOption/raceOption-line.component';
...
export class RaceListComponent {
    ...
    example : Race = new Race();

    // list is paginated
    currentPage : PageResponse<Race> = new PageResponse<Race>(0,0,[]);

    // X to one: input param is used to filter the list when displayed
    // as a one-to-many list by the other side.
    private _raceOpt : RaceOption;
    ...
    loadPage(event : LazyLoadEvent) {
        this.raceService.getPage(this.example, event).
            subscribe(
    ...
    set raceOpt(raceOpt : RaceOption) {
        ...
        this._raceOpt = raceOpt;

        this.example = new Race();
        this.example.raceOpt = new RaceOption();
        this.example.raceOpt.id = this._raceOpt.id;
    }
    ...
    showDeleteDialog(rowData : any) {
        let raceToDelete : Race = <Race> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(raceToDelete);
            }
        });
    }

    private delete(raceToDelete : Race) {
        let id =  raceToDelete.id;

        this.raceService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(raceToDelete);
