...
export const SKILL_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR: any = {
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => SkillCompleteComponent),
    multi: true
};
	...
	template: `
        <p-autoComplete [(ngModel)]="value" [disabled]="disabled" placeholder="Hint: type to search..." field="name" [suggestions]="suggestions" (completeMethod)="complete($event)" (onSelect)="select($event)">
            <ng-template let-skill pTemplate="item">
                <skill-line [skill]="skill"></skill-line>
            </ng-template>
        </p-autoComplete>
	`,
	selector: 'skill-auto-complete',
    providers: [SKILL_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR]
})
export class SkillCompleteComponent implements ControlValueAccessor {
    @Input() disabled : boolean = false;
    @Input() id : string;
    @Input() name : string;

    //The internal data model
    private _value: Skill = null;

    public suggestions : Skill[] = [];
    ...
    complete(event:any) {
        this.skillService.complete(event.query).
            subscribe(
