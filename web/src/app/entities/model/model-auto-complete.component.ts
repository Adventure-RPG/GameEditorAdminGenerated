...
export const MODEL_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR: any = {
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => ModelCompleteComponent),
    multi: true
};
	...
	template: `
        <p-autoComplete [(ngModel)]="value" [disabled]="disabled" placeholder="Hint: type to search..." field="name" [suggestions]="suggestions" (completeMethod)="complete($event)" (onSelect)="select($event)">
            <ng-template let-model pTemplate="item">
                <model-line [model]="model"></model-line>
            </ng-template>
        </p-autoComplete>
	`,
	selector: 'model-auto-complete',
    providers: [MODEL_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR]
})
export class ModelCompleteComponent implements ControlValueAccessor {
    @Input() disabled : boolean = false;
    @Input() id : string;
    @Input() name : string;

    //The internal data model
    private _value: Model = null;

    public suggestions : Model[] = [];
    ...
    complete(event:any) {
        this.modelService.complete(event.query).
            subscribe(
