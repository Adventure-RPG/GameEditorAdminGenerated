...
export class ModelListComponent {
    ...
    example : Model = new Model();

    // list is paginated
    currentPage : PageResponse<Model> = new PageResponse<Model>(0,0,[]);
    ...
    loadPage(event : LazyLoadEvent) {
        this.modelService.getPage(this.example, event).
            subscribe(
    ...
    showDeleteDialog(rowData : any) {
        let modelToDelete : Model = <Model> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(modelToDelete);
            }
        });
    }

    private delete(modelToDelete : Model) {
        let id =  modelToDelete.id;

        this.modelService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(modelToDelete);
