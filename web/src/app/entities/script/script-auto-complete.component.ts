...
export const SCRIPT_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR: any = {
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => ScriptCompleteComponent),
    multi: true
};
	...
	template: `
        <p-autoComplete [(ngModel)]="value" [disabled]="disabled" placeholder="Hint: type to search..." field="name" [suggestions]="suggestions" (completeMethod)="complete($event)" (onSelect)="select($event)">
            <ng-template let-script pTemplate="item">
                <script-line [script]="script"></script-line>
            </ng-template>
        </p-autoComplete>
	`,
	selector: 'script-auto-complete',
    providers: [SCRIPT_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR]
})
export class ScriptCompleteComponent implements ControlValueAccessor {
    @Input() disabled : boolean = false;
    @Input() id : string;
    @Input() name : string;

    //The internal data model
    private _value: Script = null;

    public suggestions : Script[] = [];

    //Placeholders for the callbacks
    private _onTouchedCallback: () => void = () => {};
    private _onChangeCallback: (_:any) => void = () => {};

    constructor(private scriptService : ScriptService, private messageService : MessageService) {
    }
    ...
    complete(event:any) {
        this.scriptService.complete(event.query).
            subscribe(
